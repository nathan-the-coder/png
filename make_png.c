...
/* Write "bitmap" to a PNG file specified in "path"; return 0 on
...
static int save_png_to_file(bitmap_t *bitmap, const char *path) {
  ...
  /* "status" contains the result value of this function. At first
   * it is set to a value which means 'failure'. When the routine
   * has finished its work, it is set to a value which means
   * 'success'. */
  int status = -1;

  /* The following number is set by trial and error only. I don't
   * know where it is documented in the libpng manual.
   */
  int pixel_size = 3;
  int depth = 8;
